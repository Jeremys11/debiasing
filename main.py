...
def main():
    ...
    temp_airnow = airnow_data.reset_index(drop=True)
    temp_forecast = forecast_data["O3"][my_location,:].reset_index(drop=True)

    graph_airnow = temp_airnow[my_location][start_day:end_day]
    ...
    pyplot.savefig("Output/Eval.png")
    pyplot.clf()
    ...
    r_base, p_value1 = stats.pearsonr(graph_prediction,graph_airnow)
    r_forecast, p_value2 = stats.pearsonr(graph_forecast,graph_airnow)
    print("airnow - Resnet Correlation: ", r_base)
